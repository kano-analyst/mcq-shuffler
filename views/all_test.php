        ...
        <th>Delete</th>
            ...
            while($row = mysqli_fetch_assoc($result)) {
               ...
               $query2= "SELECT * FROM `mcq` where `testId`='$testId'";
                ...
                $url="../controllers/delete_test_handler.php?id=".$testId;
                echo '<tr><td>'.$numbering.'</td><td>'.$row['department'].'</td><td>'.$row['course'].'</td>
                <td>'.$row['code'].'</td><td>'.$row['date'].'</td><td>'.$row['author'].'</td>
                <td>'.$affected.'</td><td><a class="btn btn-default" href="view_test.php?testId='.$testId.'">View</a>
               </td><td><a style="position:center" href="../controllers/delete_test_handler.php?id='.$testId.'"class="btn btn-danger" onclick="ConfirmDelete()" >Delete</a></td>';
                                                echo '</tr>';
                                                $numbering++;
